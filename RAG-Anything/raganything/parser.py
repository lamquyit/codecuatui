...
class Parser:
    ...
    IMAGE_FORMATS = {".png", ".jpeg", ".jpg", ".bmp", ".tiff", ".tif", ".gif", ".webp", ".heic"}
    TEXT_FORMATS = {".txt", ".md"}
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        try:
            ...
            # Filter out metadata-only kwargs that are not for parsing
            # These are for chunk metadata, not for mineru command
            metadata_keys = {'role_ids', 'com_id', 'department', 'security_level', 'doc_name'}
            parser_kwargs = {k: v for k, v in kwargs.items() if k not in metadata_keys}
    ...
    def parse_image(
        ...
        **kwargs,
        ...
        try:
            ...
            # Filter out metadata-only kwargs
            metadata_keys = {'role_ids', 'com_id', 'department', 'security_level', 'doc_name'}
            parser_kwargs = {k: v for k, v in kwargs.items() if k not in metadata_keys}

            try:
                # Run mineru command (use temp file if conversion happened)
                self._run_mineru_command(
