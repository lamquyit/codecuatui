...
class RAGAnything(QueryMixin, ProcessorMixin, BatchMixin):
    ...
    def _initialize_processors(self):
        ...
        if getattr(self.config, "enable_handwriting_processing", False):
            self.modal_processors["handwriting"] = VinternProcessor(
                lightrag=self.lightrag,
                modal_caption_func=None, # Vintern does its own OCR
                context_extractor=self.context_extractor,
            )
    ...
    async def _ensure_lightrag_initialized(self):
        ...
        try:
            # Check parser installation (warning only — parser is needed for indexing, not query)
            if not self._parser_installation_checked:
                if not self.doc_parser.check_installation():
                    self.logger.warning(
                        f"Parser '{self.config.parser}' is not found in PATH. "
                        "Document indexing will not work, but querying existing data is OK."
                    )
                else:
            ...
            try:
                ...
                # [RBAC] Add role_ids and company_id to chunks_vdb meta_fields so NanoVectorDB persists them
                if hasattr(self.lightrag, 'chunks_vdb') and hasattr(self.lightrag.chunks_vdb, 'meta_fields'):
                    self.lightrag.chunks_vdb.meta_fields.add("role_ids")
                    self.lightrag.chunks_vdb.meta_fields.add("company_id")
                    self.logger.info(f"Extended chunks_vdb meta_fields: {self.lightrag.chunks_vdb.meta_fields}")
