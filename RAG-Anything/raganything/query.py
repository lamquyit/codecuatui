...
class QueryMixin:
    ...
    async def rbac_query(self, query: str, user_roles: list[str], mode: str, system_prompt: str = None, company_id: str = None, **kwargs):
        """
        Execute query with Role-Based Access Control
        1. Embed Query
        2. Retrieve chunks from chunks_vdb
        3. Filter chunks by company_id (HARD filter)
        4. Filter chunks by role_ids (SOFT filter)
        5. Generate Answer
        """
        self.logger.info(f"Executing RBAC query for company={company_id}, roles={user_roles}")
        
        # 2. Retrieve ALL candidates from VDB
        #    NanoVectorDBStorage.query(query: str, top_k, ...) handles embedding internally
        top_k = kwargs.get("top_k", self.config.top_k if hasattr(self.config, "top_k") else 10)
        
        # Fetch a large number so post-filtering by company/role doesn't miss small tenants
        fetch_k = top_k * 50  # large multiplier to cover all tenants
        
        try:
             results = await self.lightrag.chunks_vdb.query(query, top_k=fetch_k)
        except Exception as e:
             self.logger.error(f"Failed to query chunks_vdb: {e}")
             return "Sorry, I encountered an error while retrieving information."
        
        # 3. Filter by Company (HARD filter - must match exactly)
        # 4. Filter by Role (SOFT filter - intersection)
        filtered_results = []
        for res in results:
              chunk_data = res
              
              # Company filter: STRICT — only return chunks that exactly match company_id
              if company_id:
                  chunk_company = chunk_data.get("company_id")
                  if chunk_company != company_id:
                      continue
              
              # Role filter: check intersection
              allowed_roles = chunk_data.get("role_ids", ["public"])
        ...
        if not filtered_results:
            return "No relevant information found authorized for your roles."
            
        # 4. Generate Answer
        context = "\n\n".join([c.get("content", "") for c in filtered_results])
        ...
        # Call LLM
        answer = await self.lightrag.llm_model_func(rag_prompt)
        return answer

    async def aquery(
        self, query: str, mode: str = "mix", system_prompt: str | None = None, user_roles: list[str] = None, company_id: str = None, **kwargs
    ) -> str:
        """
        ...
        if user_roles:
             return await self.rbac_query(query, user_roles, mode, system_prompt, company_id=company_id, **kwargs)
