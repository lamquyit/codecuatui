...
class VinternProcessor(BaseModalProcessor):
    """Processor specialized for handwriting content using Vintern OCR"""
    ...
    async def process_multimodal_content(
        ...
        **kwargs,
        ...
        """Process handwriting content using Vintern OCR"""
        
        # Get image path
        img_path = modal_content.get("img_path", "")
        if not img_path:
            logger.warning("No image path found for handwriting content")
            return "", {}, []

        entity_name = f"handwriting_{Path(img_path).stem}_{int(time.time())}"
        
        # Call Vintern for OCR
        try:
            import asyncio
            ocr_text = await asyncio.to_thread(self.client.ocr, img_path)
        except Exception as e:
            logger.error(f"Vintern OCR failed: {e}")
            ocr_text = "(Unreadable handwriting)"

        description = f"Handwriting Content:\n{ocr_text}"
        summary = f"Handwriting block containing text: {ocr_text[:50]}..." if len(ocr_text) > 50 else f"Handwriting: {ocr_text}"
        ...
        # Create chunks and graph nodes
        chunk_results = []
        if ocr_text:
            _, _, chunk_res = await self._create_entity_and_chunk(
                modal_chunk=description,
        ...
        return description, entity_info, chunk_results
    ...
    async def generate_description_only(
        ...
        """Generate description for handwriting content"""
        img_path = modal_content.get("img_path", "")
        if not img_path:
            return "", {}
        ...
        description = f"Handwriting Content:\n{ocr_text}"
        summary = f"Handwriting block: {ocr_text[:50]}..." if len(ocr_text) > 50 else f"Handwriting: {ocr_text}"
        
        entity_name = entity_name or f"handwriting_{Path(img_path).stem}"
