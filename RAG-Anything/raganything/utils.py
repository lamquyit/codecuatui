...
def convert_heic_to_png(heic_path: str) -> str:
    """
    ...
    Returns:
        str: Path to the converted PNG file, or empty string if conversion fails
    """
    try:
        ...
        logger.info(f"Converting HEIC file to PNG: {heic_path} -> {output_path}")
        ...
        logger.error("pillow-heif is required for HEIC conversion. Please install it with: pip install pillow-heif")
        return ""
    except Exception as e:
        logger.error(f"Failed to convert HEIC to PNG {heic_path}: {e}")
        return ""
